...
///////////////////////
{
	...
	// dev-only channel solo (V2SEQ_SOLO env; absent from NDEBUG builds): keep
	// one channel's emitted MIDI, discard the rest -- timing bookkeeping
	// untouched. The portable port of the lab's CHANSOLO bisection tool.
#ifndef NDEBUG
	static int chansolo = -2;
	if (chansolo == -2) { const char *e = getenv("V2SEQ_SOLO"); chansolo = e ? atoi(e) : -1; }
#endif
	...
	for (sInt ch=0; ch<16; ch++)
	{
		...
		if (!bc.notenum)
			continue;
#ifndef NDEBUG
		sU8 *solo_mptr0 = mptr; sU32 solo_laststat0 = laststat;
#endif
