...
#include "v2eras.h"   // v2redux::Era (engine-identity coordinate)
...
public:
  ...
  void SetSourceVersion(sInt a_ver) { m_era = (a_ver < 0) ? v2redux::Era::Auto()
                                                          : v2redux::Era::v(a_ver); }

  // era compat (richer): set the full Era coordinate -- lets the player pin a
  // build the format version cannot express (e.g. v2redux::eras::kkrieger2004).
  void SetEra(const v2redux::Era &a_era) { m_era = a_era; }
